...
> **우선순위 변경 (2026-07-29).** 레퍼런스로 비주얼 톤을 맞추는 것은 뒤로 미룬다. 지금 확정해야 하는 것은 **구조와 UI 위치**이고, 그건 이미지 AI보다 와이어프레임이 정확하다 → [DESIGN.md](DESIGN.md) 14.2.1 밴드 명세.
>
> 이 문서의 프롬프트는 **밴드 명세가 정해진 뒤 질감·분위기를 입힐 때** 쓴다. 배치를 이 프롬프트로 탐색하지 말 것 — 이미지 AI는 비율을 지키지 않는다.
