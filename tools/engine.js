// ── UNSCRIPTED · UNDERSTUDY · UNCAST — 규칙 엔진 v2 (대본 구조) ──
//
...
(function (root) {
  ...
  var DIFFICULTY = {
    ...
    // 0.7 일 때 처음 86% · 익숙/숙련 95% 로 사실상 실패가 없었다. 1.2 로 올리면
    // 처음 75% · 무작정 12% 다 — 넷 중 하나는 두 번째 판에 깬다.
    //
    // 더 조여도 소용없다는 것을 실측했다: 1.2/1.6 까지 올려도 숙련은 95% 에서 안 내려오고
    // 무작정만 18% → 2% 로 무너진다. 적 강도는 이야기 난이도에서 「틀린 사람을 때리는」 지렛대다.
    // actHp 를 1.35 배로 올리면서 이야기까지 같이 어려워져 무작정이 10% 로 무너졌다.
    // 이 자리는 「엔딩 보러 온 사람이 두세 판이면 닿는다」가 확정이라 정확히 상쇄한다.
    story:  { name: '스토리', hpMul: 0.42, atkMul: 0.8,
              note: '결말을 보러 왔다', sub: '두세 판이면 닿는다. 실패해도 이야기는 이어진다.' },
    ...
    normal: { name: '보통',   hpMul: 2.55, atkMul: 2.75,
              note: '한 번 이겨보려 한다', sub: '첫 클리어가 사건이다. 죽고 다시 하면서 배운다.' },
    // 재연 1단이 곧 이 난이도다. 3.3 / 2.7 로는 클리어가 15% 라 재연이 거의 올라가지 않았다.
    // 공격 ×2.05 가 보통(×2.75)보다 낮아 HP 우위와 서로 지웠다 — 보통과 구분이 안 됐다
    hard:   { name: '어려움', hpMul: 3.2,  atkMul: 2.40,
              note: '이미 이겨본 사람의 자리', sub: '운과 실력이 함께 있어야 한다. 3막을 넘기면 재연이 열린다.' }
  };
  ...
  var EVENTS = [
    ...
    { id: 'audition', name: '대역 오디션', icon: '🎬',
      ...
      opts: [['같은 배역 2장을 1장으로 — 그 배역 1장을 더 강한 것으로', 'merge'],
             ['둘 다 남긴다', 'none']] },
    // ── 판 중간 가속 (53.2) ──────────────────────────────
    // 무대 조건이 「모으고 플레이한 누적」뿐이면 시간이 곧 조건이 된다.
    // 건너뛰기가 있어야 판마다 속도가 달라지고 「이번 판은 되겠는데?」가 생긴다.
    { id: 'invite', name: '초대장',     icon: '🎫',
      text: '봉인된 초대장이 놓여 있다. 받는 사람 이름이 비어 있다.',
      opts: [['연다 — 노리는 무대의 조건 하나가 즉시 채워진다', 'invite'],
             ['두고 간다', 'none']] },
    { id: 'folio',  name: '낡은 대본철', icon: '📜',
      text: '먼지 낀 대본철. 본 적 없는 연출이 적혀 있다.',
      opts: [['읽는다 — 아직 못 본 무대 하나가 도감에 등록된다', 'folio'],
             ['덮는다', 'none']] },
  ...
  var CFG = {
    ...
    hpBase: 60, hpPerAct: 42, costPerAct: 1,
    // 분장실 — 회복이 소모전을 통째로 되돌리고 있었다 (숙련 기준 깎임 70 · 회복 53)
    restAt: 0.72, restHeal: 0.25,
    // 방어가 전투 시작에 한 번만 0 이 되고 그 뒤로는 쌓이기만 했다. 적은 쿨타임 2 라
    // 두 턴에 한 번 때리므로, 두 턴치 방어로 한 번의 공격을 받는 구조였다 —
    ...
    // 전투가 2.4턴이라 적 대부분이 한 번 때리기 전에 퇴장했다. 쿨타임이 2 이고
    // 1턴에는 움직이지 않으므로, 짧은 전투에서는 공격력을 올려도 때릴 기회가 없다 —
    // 공격 배율을 45% 올렸을 때 숙련 클리어가 6pp 움직인 반면, HP 를 35% 올리자
    // 23pp 움직였다. 전투가 2.9턴이 되고 전투당 들어오는 피해가 11 → 14 가 된다.
    actHp: [0.81, 1.35, 1.76], actAtk: [0.85, 1.05, 1.25],
    // 골드 — 남으면 결정이 아니다.
